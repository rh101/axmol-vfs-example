...
namespace myapp
{
class AX_DLL VfsFileUtils : public ax::FileUtilsWin32
{
public:
    static VfsFileUtils* create();

    VfsFileUtils();
    ~VfsFileUtils() override;
    ...
    std::unique_ptr<ax::IFileStream> openFileStream(std::string_view filePath, ax::FileStream::Mode mode) const override;
